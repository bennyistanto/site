---
title: "Two ways to see a drought, and what they are made of"
...
Now notice the asymmetry in the numbers. Rainfall fell by roughly 90 per cent of a small quantity. Demand rose by roughly 10 per cent of a large one. **In absolute millimetres the increase in demand is comparable to the entire normal rainfall of the month.** An index that tracks only the rainfall reports the first column and is silent about the second.
...
The supply side is the **Standardized Precipitation Index**, SPI, computed here at 1, 3, 6 and 12 month accumulations. The demand side is the **Evaporative Demand Drought Index**, EDDI, at the same four. Both from TerraClimate v1.1, both 1950 to 2025, both against the same 1991-2020 baseline. Same input, same period, same calibration, so a difference between them is a difference in what they measure rather than in how they were built.

There is an obvious third candidate, and it is deliberately not used.

The Standardized Precipitation Evapotranspiration Index takes precipitation *minus* reference evapotranspiration and standardises the difference. It is a good index and it is widely used. It is the wrong index here, because it already contains the demand term. Pairing SPEI with EDDI would put reference ET on both axes and then report the resulting correlation as a finding.
...
If the index is a rank among $k$ baseline years, then there are only $k+1$ possible answers, and the most extreme of them is bounded. Compute the bound rather than assume it:
...
NOAA's EDDI classification has eleven categories, and on a thirty-year baseline **the outermost class at each end, EW4 and ED4, cannot be reached at all**. ED4 begins at the 98th percentile, and a thirty-year rank tops out at the 97.87th. This is not a matter of those categories being rare or unlikely. There is no arrangement of the data that produces one.

That follows from the baseline this project chose and says nothing against NOAA's scheme. NOAA standardises EDDI against the whole period of record instead of a thirty-year normal: PSL uses 1980-2015, reaching the 98.21st percentile, and CPC uses 1980-2021, reaching the 98.46th. Both clear the 98th, so both can report an exceptional drought. The WMO 1991-2020 window used here cannot, and that is the price of making this product line up with everything else in the series.

This matters for what comes later. During a strong El Niño a non-trivial share of cells will sit exactly at the +2.03 bound, and a cell already at the bound cannot get worse in the index even when conditions do. Post 7 finds two of the five case cells pinned there in August 2026. A fit run against that flattened top end comes out shallower than the truth, because part of its evidence has nowhere left to move.
...
This is not a small correction to be waved at. It is the single largest source of error in this project, and it caught me three separate times in three different disguises, described in the final post. Every later post that attributes something to El Niño removes this trend first and reports the before-and-after, because the difference between them is often larger than the effect being claimed.
...
| **AgERA5 reference ET** | **0.1°** | **1979-01 to 2026-08** |
| **AgERA5 EDDI-3** | **0.1°** | **1979-01 to 2026-08** |
| RONI, ONI | Niño 3.4 | 1950-01 to 2026-06 |
| CPC RONI outlook | 9 seasons | 2026-07 onward |

Those spans are read out of the files rather than quoted from a plan.

The two rows in bold are the reason this series can say anything about the present. **TerraClimate stops at December 2025**, eight months before the event began. **AgERA5 reaches August 2026**, with 2.33 million land cells in that month. One dataset is the history and the other is the news, and post 7 is written entirely from the second.

The land data carries `version V1.1`, `date_created 2026-06-02` and `source WorldClim v2.1, ERA5`, from the University of California Merced, after Abatzoglou, Hegewisch, Yin and Kalashinkov (2026).

## TerraClimate is not an observation

TerraClimate is built by taking anomalies from a **reanalysis**, ERA5, and applying them to a high-resolution climatology, WorldClim v2.1. A reanalysis is a weather model run back over the past, pulling in every observation it can find as it goes, so what comes out is an estimate that obeys physics everywhere, including in the places where nobody was measuring. It is an interpolation, not a measurement. No instrument recorded 4.6 km reference evapotranspiration in the Sahel in 1953.
...
Drought has two halves, and at Indramayu in 1997 the rise in demand was worth about as many millimetres as the whole normal rainfall of the month.
...
*Next: [Seventy-five years of dry spells](20260825-seventy-five-years-of-dry-spells.qmd), applying run theory to both indices at four accumulation lengths, to find out what El Niño actually changes.*
