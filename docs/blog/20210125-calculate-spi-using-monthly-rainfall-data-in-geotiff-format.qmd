...
date: "2021-01-25"
description: "A full walkthrough for calculating the Standardized Precipitation Index straight from monthly rainfall GeoTIFFs, using CDO, NCO and the climate-indices package"
image: "../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-02.jpg"
categories:
  - "Remote Sensing"
  - "Research"
  - "Climate"
...
These last few months, I have tried a lot of difference formulation to calculate Standardized Precipitation Index ([SPI](https://drought.unl.edu/droughtmonitoring/SPI.aspx)) based on rainfall data in [netCDF](https://www.unidata.ucar.edu/software/netcdf/) format, check below blog post as a background:

1. SPI using [IMERG](/blog/20200706-calculate-spi-using-imerg-data)
2. SPI using [CHIRPS](/blog/20200710-calculate-spi-using-chirps-data)

The reason why I use rainfall in netCDF format in above blog post because the software to calculate SPI: [climate-indices](https://pypi.org/project/climate-indices/) python package will only accept single netCDF as input, and the SPI script will read the netCDF input file based on time dimension.
...
Then I found this [solution](/blog/20201210-geotiff-to-netcdf-file-with-time-dimension-enabled-and-cf-compliant).
...
1.5. Download and install Anaconda Python on your machine: [macOS](https://repo.anaconda.com/archive/Anaconda3-2020.11-MacOSX-x86_64.pkg), [Windows](https://repo.anaconda.com/archive/Anaconda3-2020.11-Windows-x86_64.exe), [Linux](https://repo.anaconda.com/archive/Anaconda3-2020.11-Linux-x86_64.sh).
...
2.4. Install Climate Data Operator ([CDO](https://code.mpimet.mpg.de/projects/cdo)) from Max-Planck-Institut für Meteorologie using conda

  ```bash
  conda install -c conda-forge cdo
  ```

2.5. Install netCDF Operator ([NCO](http://nco.sourceforge.net/)) using conda
...
3.5. After everything is set, then you can execute the translation process
  ...
  * Using Python in Terminal, navigate to your `Script` directory, type `python tiff2nc.py`
  ...
  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-02.jpg)

  * Wait for a few moments, you will get the output java\_cli\_chirps\_1months\_1981\_2020.nc. You will find this file inside `Input_TIF` folder. Move it to `Input_nc` folder.
  * Using Jupyter, make sure you still inside conda `climate_indices` environment, type in Terminal `jupyter notebook`

  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-03.jpg)

  * Navigate to your notebook directory (where you put \*.ipynb file), run Cell by Cell until completed. Wait for a few moments, you will get the output java\_cli\_chirps\_1months\_1981\_2020.nc. You will find this file inside `Input_TIF` folder. Move it to `Input_nc` folder.
  * You also can get this data: java\_cli\_chirps\_1months\_1981\_2020.nc via this link <http://on.istan.to/3iRDl7e>
...
4.1. I have try to make script in Step 3.4. are follow above specification. To make sure everything is correct, in your Terminal - navigate to your directory where you save java\_cli\_chirps\_1months\_1981\_2020.nc file: `Input_nc`. Then type `ncdump -h java_cli_chirps_1months_1981_2020.nc`

  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-04.jpg)
...
4.2. In order to pre-compute fitting parameters for later use as inputs to subsequent SPI calculations I can save both gamma and Pearson distribution fitting parameters to NetCDF, and later use this file as input for SPI calculations over the same calibration period.
  ...
  * Monthly IMERG data, global coverage 180W - 180E, 60N - 60S, 0.1 deg spatial resolution. It takes almost 9-hours to process SPI 1-72 months.
  ...
  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-09.jpg)
...
![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-08.jpg)

### 6. Visualized the result using [Panoply](https://www.giss.nasa.gov/tools/panoply/)

Let see the result.

6.1. From the `Output_nc` directory, right-click file java\_CHIRPS\_spi\_gamma\_3\_month.nc (you can download this file from this link: <https://on.istan.to/2MhVnTP>) and Open With Panoply.

  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-05.jpg)

6.2. From the Datasets tab select spi\_gamma\_3\_month and click Create Plot

  ![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-06.jpg)
...
6.4. When the Plot window opens:
  ...
  * Scale tab: Change value on Min -3, Max 3, Major 6, Color Table CB\_RdBu\_09.cpt
  ...
  * Overlays tab: Change Overlay 1 to MWDB\_Coasts\_Countries\_1.cnob

![](../assets/image-blog/20210125-calculate-spi-using-monthly-rainfall-data-in-geotiff-format-01.jpg)
