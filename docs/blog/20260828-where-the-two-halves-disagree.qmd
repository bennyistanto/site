---
title: "Rain and thirst move together under El Niño, which is worse news"
...
Counting dry spells on each axis separately leaves the obvious question unasked. The two axes exist because of one specific hypothesis, and putting them together is how it gets tested.
...
What is there instead is a *compound hazard*. On nearly two thirds of the land where the response is solid, El Niño reduces the water arriving and increases the water leaving, **at the same time, in the same season**. What that costs on the ground is outside what this data can see. There is no soil moisture in the analysis, and post 9 is explicit that the path from an index to an impact runs through soil, planting date and irrigation, none of which is here. What can be said is narrower, and it still matters: the two shortfalls arrive in the same season, not in different ones, so a monitor watching either axis alone catches one and misses the other.
...
Everything so far has been the historical record. The next post stops looking backwards and asks where the world actually is, with the event already under way. It waits for August to close first, and then a little longer: the observations it needs do not exist until the month is over, and the reanalysis they come from runs about eight days behind.

*Next: [Where the world is dry right now, in rainfall and in thirst](20260908-where-it-is-right-now.qmd), once August has closed.*
