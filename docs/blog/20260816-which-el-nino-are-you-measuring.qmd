---
title: "Which El Niño are you measuring? The index changed this year"
...
Effective February 2026, NOAA's Climate Prediction Center adopted the Relative Oceanic Niño Index for official monitoring and prediction of ENSO, the El Niño-Southern Oscillation. That is the name for the whole cycle: El Niño at the warm end, La Niña at the cool end, and the neutral state that occupies about half of the record in between. Its [information circular](https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/enso/roni/pdf/roni-info-may-2026.pdf) states that RONI "is now the primary standard for official advisories and classifications of ENSO events", and that historical episodes are being re-evaluated with it.
...
The **Relative Oceanic Niño Index** starts from the same Niño 3.4 anomalies, subtracts the average anomaly across the global tropical belt, 20°S to 20°N, taken against the current standard normal of 1991-2020 set by the World Meteorological Organization, and then rescales the difference so its overall variability matches ONI's:
...
thirty-year window forward every five years; RONI uses the current WMO
standard normal, 1991-2020 today. The circular's claim is that RONI is *less
sensitive* to which thirty-year period is chosen, not that it escapes the
choice altogether.
...
What the difference between them really is: a mixture of the tropical-belt subtraction, the variance rescaling, and the fact that the two indices use different reference periods. Measured, it is dominated by the trend and carries almost none of the ENSO signal itself. The data supports that much and no more, which is enough for what follows.
...
That needs something on the land to test against. The measure used here is **EDDI**, the Evaporative Demand Drought Index: how unusual the atmosphere's thirst is for the time of year at that place, worked out from temperature, humidity, wind and sunshine. A high EDDI means a thirsty atmosphere. Post 4 builds it from scratch and explains why it is half of what a drought is; for now that one sentence is enough.
...
The map is one-signed almost everywhere. Orange, where RONI gives the lower coefficient, covers the Sahara, the Sahel, central Africa, Amazonia, interior Asia and most of North America. Purple is confined to peninsular India, parts of the Maritime Continent, and interior Australia. The Maritime Continent is the belt of islands and shallow seas between the Indian and Pacific oceans, running from Sumatra east to New Guinea; this series takes it as the box from 11°S to 8°N and 95°E to 141°E, and it turns out to be where most of the signal lives.
...
The relationship is proportional, and the fitted slope is 0.080, well short of one to one. The median response to the differing component is **+1.00 EDDI units per °C**, positive on **95.0%** of land, and that near-universal positive response is why the map is one-signed. Almost everywhere gets thirstier when the removed component rises, so almost everywhere has something to remove.

A consistency check, then, and not a discovery. What it confirms is that the shift is systematic and mechanical, with no noise in it. It does not by itself say which index is the better description of the ocean; CPC's adoption decision does that.

There is more to say about what that mechanism largely **is**. The component separating the two indices is trend-dominated, as the table above shows at R² = 0.45 against time. Evaporative demand is also trending, on most of the world, as post 4 will show. Measured across the same cells, a pixel's response to the removed component correlates with that pixel's own EDDI trend at **+0.888**. So "almost everywhere gets thirstier when the removed component rises" is, to a large degree, two series rising together over seventy-six years. That is a real reason to prefer the index without the trend in it, which is the case for RONI. It is not evidence of a physical response to the tropical mean, and nothing on this page has been detrended. Whether RONI's advantage in finding signal survives detrending is a question this series does not test.
...
in the [Benjamini-Hochberg](https://doi.org/10.1111/j.2517-6161.1995.tb02031.x) form used here at q < 0.05, sets the bar higher the
...
The Maritime Continent is a different world from the global average. Seventy per cent of its cells carry a signal, and **47% survive leave-one-event-out cross-validation** against a global figure of 1.5%. That test hides one El Niño, refits the relationship without it, and asks the fit to predict the event it never saw. Surviving it is a much higher bar than merely fitting the past, and post 9 is largely about how few places clear it. The strongest cell, near Luwuk in Indonesia, gives r = +0.68, +0.73 EDDI units per °C, and a cross-validated R² of +0.45. The relationship there is genuine and checkable, well clear of marginal.
...
*Next: [Forty-four ENSO events, and only eight that can answer anything](20260819-forty-four-events.qmd).*
