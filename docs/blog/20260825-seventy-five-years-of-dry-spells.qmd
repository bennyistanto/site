---
title: "El Niño starts dry spells, it does not make them longer"
...
Two indices are now on the table, measuring different halves of the same balance. That makes it possible to ask the question sitting behind the whole series: **over seventy-five years, what does El Niño actually change about dry spells?**

Not the average anomaly. Dry spells: how often one starts, how long it lasts, how deep it gets.

## What counts as a spell

A **run** is a continuous stretch where an index stays beyond a threshold. The framework is [Yevjevich's](https://mountainscholar.org/items/caa939a5-98a9-4ca3-8ea3-308c310cdad7), set out in Hydrology Paper 23 at Colorado State University in 1967, and it is still the standard way to turn a drought index into countable events. Dry is a **low SPI** and a **high EDDI**, so the same physical condition is `SPI < −1.0` and `EDDI > +1.0`. Two consecutive months qualify; one does not.
...
Crediting at the end instead would let a spell that began under El Niño and outlived it count as evidence about neutral conditions. Runs still open at December 2025 are discarded, never reported at their truncated length.
...
Nearly every number from here on is a comparison written that way, so here is
...
Two consequences follow into the figures. Neutral is the yardstick rather than "all months", because all months would include the El Niño ones and so dilute the contrast against itself. And **La Niña's 224 months take no part in any of these numbers**.

That second choice needs defending, because post 4 promised that every post attributing something to El Niño would take the warming trend out first, and this one does not. The reason is in the dates. El Niño months average **1984.5** and neutral months **1983.9**, about six months apart, which at the measured trend of +0.115 EDDI per decade is worth 0.007 index units: nothing. La Niña months average **1998.0**, fourteen years later, and would inherit roughly **+0.16** from timing alone. So the El Niño against neutral contrast is safe undetrended and the La Niña one is not, which is why La Niña is left out here and gets its own detrended treatment elsewhere in the project.
...
Two rows of that table still matter.

The demand axis carries the stronger signal at every matching length: 1.14 against 1.08 at three months, 1.09 against 1.05 at one, 1.10 against 1.06 at six. Up to here the case for measuring evaporative demand has been an argument. Here the data makes it without help.
...
The Sahel is the quiet row, at +1.8 and +3.2. Worth remembering if the Sahel turns up dry this season: seventy-five years of record does not attribute much of that to El Niño.
...
Southwest North America again sits in the opposite corner: fewer spells started, and slightly shorter.
...
Nearly a third of land responds on both axes, and **another 31 per cent responds on exactly one**, split almost evenly between supply-only and demand-only. If either index were quietly standing in for the other, that middle band would be small. Instead it matches the agreement.

One caution on that table, because post 2 spent a section warning about exactly this shape of statistic. The one-in-a-hundred cut is a bare threshold with no significance test behind it, so some cells land in "supply only" or "demand only" because one axis cleared the line by luck. The split is suggestive rather than solid. Post 6 asks the same question with a false discovery rate test attached, and that is the version to trust.

## The five cells

![](../assets/image-blog/20260825-seventy-five-years-of-dry-spells-08.png)

These five are not a shortlist of the most striking places, and it matters that they are not. Indramayu was chosen first on external grounds, for documented 1997 and 2015 impacts at the tail of an irrigation system, then audited afterwards: it sits at the 92nd percentile globally and the 67th within Indonesia, so it sits in the middle of the Indonesian range. The other four were picked to span the national range, being the cells nearest the 95th, 75th, 45th and 15th percentiles of Indonesia's distribution, each within 45 km of a named settlement and at least 300 km from the others. Tarutung is in the set because it sits low in that range, not because it later turned out to be the exception.
...
Tarutung runs negative on both axes at every length, which is to say El Niño leaves it wetter and calmer. Post 6 explains why that one cell keeps turning up as the exception.
...
*Next: [Rain and thirst move together under El Niño, which is worse news](20260828-where-the-two-halves-disagree.qmd).*
